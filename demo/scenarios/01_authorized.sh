...
TOKEN_META="$TOKEN_DIR/authorized.json"
...
echo "[stage] Copying config + token to Jetson and BBB-01"
...
scp "${SSH_OPTS[@]}" "$TOKEN_BIN" "$BBB01_USER@$BBB01_HOST:$REPO_DIR/demo/config/tokens/authorized.bin"
...
echo "[run] Sending authorized task request from BBB-01"
